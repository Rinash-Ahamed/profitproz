...
export function Nav() {
  ...
  useEffect(() => {
    ...
    onHashChange() // Set initial hash on component mount
  ...
  return (
    ...
    >
        ...
        <div className="flex items-center gap-4">
          ...
          <nav className="hidden lg:flex items-center gap-1">
            {links.map((l) => {
              ...
              if (l.href === '/') {
                // Home is active only if on root and no hash
                isActive = path === '/' && hash === ''
              } else if (l.href.startsWith('/#')) {
                // Hash links are active if on root and hash matches
                isActive = path === '/' && hash === l.href.substring(1)
              }
              return isExternal ? (
                <a
                  key={l.href}
                ...
                <Link
                  key={l.href}
                  href={l.href}
                  onClick={l.href === '/' ? () => setHash('') : undefined}
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            className="lg:hidden bg-zinc-950/95 border-b border-zinc-800"
